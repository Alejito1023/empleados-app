...
<html lang="en">
  <head>
    ...
    <title>Edit Asistencia</title>

  </head>

  <body>
    <div class="container">
      <h1>Edit Asistencia</h1>
       <form method="POST" action="{{ route("asistencias.update" , ['asistencia' => $asistencia->id]) }}">
        ...
        <div class="mb-3">
          ...
          disabled="disabled" value="{{ $asistencia->id }}">
          <div id="codigoHelp" class="form-text">Asistencia Id.</div>
        </div>
        ...
        <select class="form-select"  id="empleado" name="code" required>
            ...
            @if ($empleado->id == $asistencia->id)
               <option  selected value="{{$empleado->id}}">{{$empleado->emp_nomb}}</option>
